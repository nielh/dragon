...
#define 	AHCI_PORT_IS_DSS	(1L <<  2)	/* Set DMA Setup FIS */
...
#define AHCI_PORT_IS_MASK \
	(AHCI_PORT_IS_RESTART | AHCI_PORT_IS_PRCS | AHCI_PORT_IS_DHRS | \
	 AHCI_PORT_IS_DSS | AHCI_PORT_IS_PSS | AHCI_PORT_IS_SDBS)
...
#define     AHCI_PORT_CMD_ICC	(1L << 28) /* Put i/f in active state */
...
#define 	AHCI_PORT_CMD_PWU	(1L <<  2)	/* PowerOn */
...
#define AHCI_MAX_PRDS   ((PAGE_SIZE - AHCI_CL_SIZE -128) / 16) // 168
...
typedef struct {
	u8 cf_cmd;		/* Command */
	u8 cf_feat;		/* Features */
	u32 cf_lba;		/* LBA (24-bit) */
	u8 cf_dev;		/* Device */
	u32 cf_lba_exp;	/* LBA (exp) (24-bit) */
	u8 cf_feat_exp;	/* Features (exp) */
	u8 cf_sec;		/* Sector Count */
	u8 cf_sec_exp;	/* Sector Count (exp) */
	u8 cf_ctl;		/* Control */
} cmd_fis_t;
